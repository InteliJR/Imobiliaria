  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (responseUpdate && responseUpdate.status === 200) {
        ...
        toast.success(responseUpdate?.data?.message || "Perfil alterado com sucesso.");
        // console.log("Perfil atualizado com sucesso");
        navigate("/perfil");  
      } else {
        setResultMessage("Erro ao atualizar o perfil");
        toast.error(responseUpdate?.data?.message || "Erro ao se conectar com o servidor.");
    ...
    <main className="main-custom">
      ...
      <section className="section-custom">
        <div className="w-full flex justify-center items-center flex flex-col">
          <form className="flex flex-col gap-4 mb-4 max-w-4xl w-full">
            <Voltar />
            <h1 className="font-bold text-lg justify-self-center self-start">
              Editar Perfil
            </h1>
            <FormField
              label="Nome"
